...
public:
    ...
    void setOrientation(const float &theta) { this->theta = theta; }

    float getAngVelocity() { return angVel; }

    void setAngVelocity(const float &angVel) { this->angVel = angVel; }
    ...
    void setType(typeOfRigidBody t) { objType = t; }
...
public:
    ...
    size_t getSize() { return list.size(); }

    RigidBody *getRigidBody(const int &idx) { return list.at(idx); }
