...
public:
    ...
    RendererInput() : fluid_mesh(nullptr) {
        ...
        dummy_mesh = std::make_unique<ColoredMesh>(
            square,
            std::vector<glm::vec3>{
                {1, 0, 0.4}, {0.3, 1, 0.4}, {1, 1, 0.4}, {1, 0, 0.4}, {1, 1, 0.4}, {0, 0, 1}});
    }

    void process(const GameLogicOutput &gameLogicOutput,
                 const PhysicsOutput &physicsOutput) { // handle game logic output
        ...
        world_objects.assign(gameLogicOutput.objectsToRender.begin(),
        ...
        for (auto &gameLogicObject : world_objects) {
            if (!gameLogicObject.mesh) {
                Log::warn(
                    "A game_logic object does not have a render mesh, rendering as rectangle");
        ...
        if (gameLogicOutput.rigid_body_meshes) {
            for (const Transform *rigidBody : rigidBodyPhysicsOutput.rigid_bodies) {
                ...
                const Mesh *mesh;
                if (iter !=
                    gameLogicOutput.rigid_body_meshes->end()) { // check if the mesh is in the map
                    mesh = iter->second;
                } else {
                    Log::info("WARNING: rigid body with id %d does not have a render mesh, "
                              "rendering as square",
                              rigidBody->id);
                    mesh = dummy_mesh.get();
                }
        ...
        if (gameLogicOutput.showMiniMap) {
            createMiniMap(physicsOutput.rigidBodyPhysicsOutput);
        }

#ifdef OPENCV_LIBS
        char score_str[20];
        snprintf(score_str, sizeof(char) * 20, "Score: %-12d", gameLogicOutput.score);
        ...
        if (latticeBoltzmannOutput.velocity.width() != 0) {
            assert(latticeBoltzmannOutput.velocity.width() ==
                   latticeBoltzmannOutput.density.width());
            assert(latticeBoltzmannOutput.velocity.height() ==
                   latticeBoltzmannOutput.density.height());
            if (fluid_input.width() == 0) {
                fluid_input = Array2D<glm::vec3>{latticeBoltzmannOutput.velocity.width() - 2,
                                                 latticeBoltzmannOutput.velocity.height() - 2};
            }
            for (int x = 0; x < fluid_input.width(); ++x)
                for (int y = 0; y < fluid_input.height(); ++y)
                    fluid_input.value(x, y) =
                        glm::vec3{latticeBoltzmannOutput.velocity.value(x + 1, y + 1) * 10.f + 0.5f,
                                  latticeBoltzmannOutput.density.value(x + 1, y + 1) * 0.5f};
        }

        Log::info("RendererInput: %d objects to render", world_objects.size());
    }

    void createMiniMap(const RigidBodyPhysicsOutput &rigidBodyPhysicsOutput) {
        ...
        if (ff_width != 0) {
            if (!ff_mesh) {
                ...
                const std::vector<glm::vec3> flag_field_quad =
                    Mesh::createRectangle(glm::vec2{-1, -1}, glm::vec2{1, 1});
