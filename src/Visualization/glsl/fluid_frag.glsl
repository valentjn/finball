...
void main() {
	...
	if (int(gl_FragCoord.x) == 0)
		out_wave = noise2D(vec2(noise_scale * normalized_coords.x, t * tscale));
	else if (int(gl_FragCoord.x) == rend_res.x / 2 - 1)
		out_wave = noise2D(vec2(noise_scale * (3 - normalized_coords.x), t * tscale));
	else if (int(gl_FragCoord.y) == 0)
		out_wave = noise2D(vec2(noise_scale * (4 - normalized_coords.y), t * tscale));
	else if (int(gl_FragCoord.y) == rend_res.y - 1)
		out_wave = noise2D(vec2(noise_scale * (1 + normalized_coords.y), t * tscale));
	else
